...
/**
 * A visitor implementation that collects events grouped by case identifier
 * during automaton traversal.
 *
 * @param T The type of the timestamp used in events, which must be
 *    comparable (e.g., Int, Long, LocalDateTime).
 * @property eventsByCase A mapping from each case identifier to the list
 *    of its associated events.
 * @property cases A set of all distinct case identifiers encountered
 *    during traversal.
 */
class EventsByCasesCollector<T : Comparable<T>> : AutomatonVisitor<T> {
    ...
    /**
     * Called after the automaton traversal ends. Currently, ensures all keys
     * from [eventsByCase] are reflected in [cases], which may be useful for
     * consistency if `visit` wasn't called.
     */
    ...
    /**
     * Visits an event during automaton traversal and groups it by its
     * [caseIdentifier].
     *
